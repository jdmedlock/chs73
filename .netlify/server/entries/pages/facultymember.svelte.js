...
const Facultymember = (0, import_index_635fbd41.c)(($$result, $$props, $$bindings, slots) => {
  let { params } = $$props;
  ...
  const facultyImage = (0, import_getCloudinaryPhoto_398b4ab6.g)(cloudinaryId);
  if ($$props.params === void 0 && $$bindings.params && params !== void 0)
    $$bindings.params(params);
  return `<section class="${"flex flex-wrap place-content-center w-full"}"><div class="${"flex flex-wrap place-content-center ml-0 md:ml-8 mt-20"}">${cloudinaryId !== "" ? `<img class="${"scale-75 md:transform-none"}"${(0, import_index_635fbd41.b)("src", facultyImage, 0)} alt="${"Classmate photo"}">` : ``}
    <div class="${"flex flex-col w-full text-3xl md:text-6xl font-semibold"}"><p class="${"text-center font-semibold w-full mt-2 text-2xl md:text-4xl"}">${(0, import_index_635fbd41.e)(name)}</p>
      <p class="${"text-center font-semibold w-full mt-2 text-xl md:text-2xl"}">${(0, import_index_635fbd41.e)(department)} - ${(0, import_index_635fbd41.e)(position)}</p>
      ${deceased === "TRUE" ? `<p class="${"text-center w-full text-xl mt-4 md:text-3xl font-normal"}">(Deceased)</p>` : ``}</div>
    ...
    <div class="${"flex flex-wrap place-content-center w-full ml-0 md:ml-8 mt-1 md:mt-2"}"><div class="${"flex place-content-center text-center w-full"}"><a href="${"/" + (0, import_index_635fbd41.e)(backPage) + "/#" + (0, import_index_635fbd41.e)(lastName.toLowerCase().replace(/\s+/g, ""))}" class="${"flex bg-orange-500 text-white active:bg-gray-700 font-bold uppercase rounded shadow hover:shadow-lg outline-none focus:outline-none h-12 m-1 px-3 md:px-6 py-1 md:py-2 text-sm md:text-base transition duration-300 ease-in-out hover:text-gray-800 hover:font-semibold transform hover:-translate-y-0 hover:scale-110"}"><div class="${"flex place-self-center"}">Back
      ...
      ${confirmed === "FALSE" ? `<div class="${"text-sm md:text-lg mt-4 ml-3 justify-self-end"}">Do you know how to contact ${(0, import_index_635fbd41.e)(firstName)} ${(0, import_index_635fbd41.e)(lastName)}? Click on Tiger Hunt!
        </div>` : ``}</div></div></section>`;
});
